...
def render(node, url, depth=0):
    ...
    if tag == 'a':
        ...
        label = re.sub(r'#{1,6} ', '', clean(text)) or node.get('aria-label', '')
        if not label:
            return ''
        return '[' + label + '](' + urljoin(url, node.get('href', '')).replace(' ', '%20') + ') '
    if tag in ('strong', 'b'):
...
OUT.write_text('\n\n---\n\n'.join(parts[:1]) + '\n\n' + tidy('\n\n'.join(parts[1:])) + '\n', encoding='utf-8')
print(json.dumps({'file': str(OUT), 'pages': len(pages), 'bytes': OUT.stat().st_size, 'words': len(OUT.read_text(encoding='utf-8').split())}, indent=2))
