...
def layout(title, body, page="home", prefix="", description="", lang="en"):
    ...
    body = re.sub(
        r"(<h[1-4]\b[^>]*>)(.*?)(</h[1-4]>)", clean_heading, body, flags=re.DOTALL
    )
    C = CHROME[lang]
...
def about():
    story = (
        '<section class="about-story container"><div class="about-visual"><span class="eyebrow">ECHOFRAME</span>'
        + MARK
        + "</div><div>"
...
def globe():
    ...
    def project(lon, lat):
        lon = math.radians(lon - 5)
        lat = math.radians(lat)
        center = math.radians(12)
        depth = math.sin(center) * math.sin(lat) + math.cos(center) * math.cos(
            lat
        ) * math.cos(lon)
        x = math.cos(lat) * math.sin(lon)
        y = math.cos(center) * math.sin(lat) - math.sin(center) * math.cos(
            lat
        ) * math.cos(lon)
        if depth < 0:
    ...
    for polygon in data["objects"]["land"]["geometries"][0]["arcs"]:
        for ring in polygon:
            ...
            if not any(p[2] > 0 for p in projected):
                continue
            paths.append(
                "M" + "L".join(f"{x:.1f},{y:.1f}" for x, y, _ in projected) + "Z"
            )
    graticule = []
    for axis in ("lat", "lon"):
        for fixed in range(-75, 180 if axis == "lon" else 90, 15):
            ...
            for varying in range(
                -180 if axis == "lat" else -90, 181 if axis == "lat" else 91, 2
            ):
                x, y, z = (
                    project(varying, fixed)
                    if axis == "lat"
                    else project(fixed, varying)
                )
                if z >= 0:
                    line += ("L" if started else "M") + f"{x:.1f},{y:.1f}"
                    started = True
                else:
                    started = False
...
def main():
    ...
    write(
        "sample-briefs.html", "Sample briefs & templates", sample_library(), "samples"
    )
    ...
    write(
        ...
        audience_page("government"),
        "government",
        description="Political intelligence for oil and gas government affairs teams. Scope research on stakeholders, policy milestones, and the evidence around operating assets.",
    )
    write(
        "distressed-debt.html",
        "Distressed debt & special situations",
        audience_page("credit"),
        "credit",
        description="Political and asset-level research for distressed-debt investors. Frame thesis questions, inspect counterparty narratives, and identify evidence worth reviewing.",
    )
    ...
    for line in services.SERVICES:
        write(
            line["slug"] + ".html",
            line["name"],
            services.service_page(intro, line["slug"]),
            "services",
        )
    write(
        "projects.html", "Bespoke projects", services.projects_page(intro), "services"
    )
    write(
        "commodities.html",
        sectors.SECTORS["commodities"]["name"],
        sectors.page(intro, "commodities"),
        "sector",
    ...
    write(
        ...
        "Worked examples",
        worked_examples_page(intro),
        "worked-examples",
    )
    write(
        "frame-bureau.html",
        "The Frame Bureau",
        frame_bureau_page(intro),
        "frame-bureau",
    )
    ...
    write(
        ...
        "Page not found",
        intro(
            "404", "That page is not here", "The page you requested could not be found."
        )
        + '<div class="container section"><a class="button" href="index.html">Return to the homepage </a></div>',
    )
    ...
    assert (
        publication_root.parent == ROOT.resolve()
    ), "Publication directory must remain inside the workspace"
    ...
    published_pages = published_pages + ["gmtl_survey.html"]
    (public / "gmtl_survey.html").write_bytes((ROOT / "gmtl_survey.html").read_bytes())
    for obsolete in public.rglob("*.html"):
        if obsolete.relative_to(public).as_posix() not in published_pages:
            assert obsolete.resolve().is_relative_to(
                publication_root
            ), "Refusing to remove a page outside public/"
            obsolete.unlink()
    files = published_pages + [
        "favicon.png",
        "assets/site.css",
        "assets/presence.css",
        "assets/depth.css",
        ...
        "assets/site.js",
        "assets/globe.svg",
        "assets/energy-horizon.png",
        "assets/hero-terminal.jpg",
        "assets/example-evidence.jpg",
        "assets/case_upgrader.jpg",
        "assets/case_terminal.jpg",
        "assets/case_docket.jpg",
        "assets/case_gasplant.jpg",
        "assets/case_thermal_hero.jpg",
    ...
    for obsolete in public.rglob("*"):
        if obsolete.is_file() and obsolete.relative_to(public).as_posix() not in files:
            assert obsolete.resolve().is_relative_to(
                publication_root
            ), "Refusing to remove a file outside public/"
            obsolete.unlink()
    for path in files:
        source = ROOT / path
    ...
    print(
        f"Built {len(GENERATED)} pages; {len(published_pages)} published plus the legacy redirect. Deployment directory: public/"
    )
