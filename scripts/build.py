...
<footer class="site-footer"><div class="container"><div class="footer-top"><div>{brand(prefix)}<p>{C['tagline']}</p><span class="eyebrow">{C['regions']}</span></div><div><h2>{C['explore']}</h2><a href="{prefix}government-affairs.html">{C['f_government']}</a><a href="{prefix}distressed-debt.html">{C['f_credit']}</a><a href="{prefix}venezuela.html">{C['f_venezuela']}</a><a href="{prefix}coverage.html">{C['f_coverage']}</a><a href="{prefix}how-it-works.html">{C['f_how']}</a></div><div><h2>{C['connect']}</h2><a href="{prefix}engagement.html">{C['f_engagement']}</a><span>contact@echoframe.co</span><a href="https://ie.linkedin.com/company/echoframing" target="_blank" rel="noopener noreferrer">LinkedIn </a></div><div><h2>{C['company']}</h2><a href="{prefix}about.html">{C['f_about']}</a><a href="{prefix}trust.html">{C['f_trust']}</a><a href="{prefix}sample-briefs.html">{C['f_samples']}</a><a href="{prefix}sources.html">{C['f_sources']}</a></div></div><div class="footer-bottom"><span>© 2026 {E(SITE['legal_name'])}{' · '+E(SITE['registration_line']) if SITE['registration_line'] else ''}</span><span><a href="{prefix}privacy.html">{C['privacy']}</a><span class="meta-dot">/</span>{C['research_mark']}</span><a href="#main">{C['top']} </a></div></div></footer></body></html>'''
