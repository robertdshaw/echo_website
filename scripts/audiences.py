...
AUDIENCES = {
    'government': {
        ...
        'title': 'Judgement you can<br><em>defend to a board</em>',
    ...
    'credit': {
        ...
        'lead_text': 'Identify a political or operating assumption in the investment case. Define the event that would establish whether it holds, then seek evidence that could challenge it as well as support it.',
        'themes': [('The political catalyst', 'Which named decision, actor, or institutional milestone matters, and over what horizon?'), ('The counterparty narrative', 'Which statements are supported by documents and independent observations? Where do accounts diverge?'), ('The recovery assumptions', 'Which political or operational assumptions need evidence, and which questions belong with legal or valuation specialists?')],
...
def proof_scope(key):
    if key == 'government':
        ...
        copy = 'We can scope a first briefing around the decision that matters most. The research establishes who holds authority, what still has to happen and which local evidence would demonstrate a change.'
        ...
        url, label = 'sample-asset-access.html', 'Asset brief awaiting redacted material'
    else:
        ...
        url, label = 'sample-thesis-review.html', 'Investment brief awaiting redacted material'
...
def audience_page(key):
    ...
    return f'''<section class="audience-hero audience-{key}"><div class="container"><div class="eyebrow">{a['label']}</div><h1>{a['title']}</h1><p>{a['description']}</p><a class="button button-coral" href="briefing.html?audience={key}">{a['cta']} </a><div class="audience-hero-caption"><span>POLICY  ACTORS  ASSETS  EXPOSURE</span><span>RESEARCH WITH A DEFINED MANDATE</span></div></div><div class="audience-hero-orbits" aria-hidden="true"><i></i><i></i><i></i></div></section><section class="section container"><div class="audience-opening"><div><div class="eyebrow">Your question, in focus</div><h2>{a['lead']}</h2><p>{a['lead_text']}</p></div><blockquote>{a['question']}<cite>RESEARCH QUESTION</cite></blockquote></div><div class="audience-themes">{themes}</div></section><section class="audience-outputs"><div class="container"><div><div class="eyebrow">A briefing with substance</div><h2>What a focused<br><em>mandate can cover.</em></h2><p>These are the shapes the work takes. What you receive is set out on <a href="services.html">what we deliver</a>, and scope is agreed in a briefing conversation.</p></div><div class="outputs-list">{outputs}</div></div></section><section class="section container audience-programme"><div><div class="eyebrow">Lead development programme</div><h2>The Venezuela programme.</h2><p>Explore the risk dimensions and decision criteria alongside the historical assessment.</p><a class="text-link" href="venezuela.html">Inside the Venezuela programme </a></div></section>{brief_structure()}{audience_depth(key)}{proof_scope(key)}<section class="audience-close"><div class="container"><div class="eyebrow">Start with the question that matters</div><h2>{a['prompt']}</h2><a class="button button-coral" href="briefing.html?audience={key}">See how we can help you </a><p>Tell us your decision and what you would need to see to judge whether our research is useful.</p></div></section>'''
