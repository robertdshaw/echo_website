...
LAYERS = [
    ...
    ('Drama theory', 'Ask whether a threat or promise is credible',
     'Examine the positions in a confrontation: what each actor wants, what they threaten, and what would make another actor comply. Compare the declared position with the options actually available.',
    ...
    ('Prospect theory', 'Examine how pressure changes behaviour',
     'Consider whether an actor sees the situation as a gain to protect or a loss to reverse. Use that as a hypothesis about risk-taking, then test it against observed decisions.',
     'A costly escalation may reflect an attempt to recover a deteriorating position. The explanation needs behavioural evidence, not an assumption about personality.'),
    ('Bayesian inference', 'Let evidence change the scenario assessment',
     'Start with explicit alternatives and prior judgments. Ask how much more likely a new observation would be under one scenario than another, then record the reason for any revision.',
    ...
    ('Revealed preference', 'Set the bar from decisions, not slogans',
...
RISKS = [
    ('Security & military risk', 'The use or threat of force, security constraints, and their implications for people, access, and continuity.'),
    ('Elite fracture risk', 'Changes in alignment among decision-makers, and what those changes could mean for authority and implementation.'),
    ('Political risk', 'The institutions, rules, and policy decisions that shape the operating and contractual environment.'),
    ('Economic & fiscal stress', 'Pressure on public finances, payment capacity, and the services or infrastructure an asset depends on.'),
    ('Geopolitical pressure', 'External alignments, sanctions decisions, and international actions that could change the available options.'),
]

SCENARIOS = [
    ('Regime consolidation', 'Authority becomes more cohesive and durable.', 'Look for evidence of alignment, implementation capacity, and control. Distinguish public expressions of unity from observed cooperation.'),
    ('Elite fracture', 'Key actors cease to act as a coherent group.', 'Look for documented departures, conflicting orders, broken commitments, or changes in who controls decisions and resources.'),
    ('Negotiated transition', 'A political settlement changes who governs and on what terms.', 'Look for credible negotiating mandates, guarantees, agreed steps, and evidence that the parties can deliver on them.'),
    ('Security-force rupture', 'A break within the security apparatus changes the balance of power.', 'Look for independently supported changes in command, compliance, or deployment. Rumour alone should not establish a rupture.'),
    ('External escalation', 'Outside action materially increases pressure or disruption.', 'Look for a specific decision and its implementation, alongside the responses it triggers. Separate capability and rhetoric from action.'),
]
...
def evidence_flow():
    return '''<section class="section container evidence-flow-section"><div class="eyebrow">From source to judgment</div><h2>How evidence moves the assessment</h2><figure class="judgment-flow"><div class="judgment-stage"><span>01 / Observe</span><strong>Reports become<br>reviewable events</strong><p>Source, actor, place, time, and competing accounts.</p></div><div class="judgment-stage"><span>02 / Interpret</span><strong>Relationships and<br>behaviour add context</strong><p>Authority, incentives, dependencies, and alternatives.</p></div><div class="judgment-stage"><span>03 / Reassess</span><strong>Scenarios meet<br>the client’s criteria</strong><p>What changed, what it means, and what needs review.</p></div><figcaption>Analytical workflow illustration. Arrows describe the reasoning sequence; they do not represent data volumes or live feeds.</figcaption></figure></section>'''
...
def risk_dimensions():
    ...
    return '''<section class="section container" id="risk-dimensions"><div class="section-heading"><div><div class="eyebrow">The country assessment</div><h2>Five dimensions<br>A more useful risk picture</h2></div><p>A single country score can hide the change that matters to your business. Read these five dimensions separately, then examine their connections.</p></div><div class="risk-dimensions">'''+rows+'''</div><p class="visual-caption">A risk score, a scenario probability, and a commercial threshold answer different questions. Any numerical reading needs its date, evidence, scale, and uncertainty attached.</p></section>'''


def scenario_framework():
    items = ''.join(f'<details><summary><span>{i:02}</span>{E(name)}</summary><div><p><strong>{E(description)}</strong></p><p>{E(evidence)}</p></div></details>' for i, (name, description, evidence) in enumerate(SCENARIOS, 1))
    return '''<section class="scenario-framework section container" id="five-scenarios"><div class="section-heading"><div><div class="eyebrow">The Venezuela scenario frame</div><h2>Five ways the<br>outlook could change</h2></div><p>Scenario odds should move with evidence. Open each pathway to see the kind of development that would prompt a review.</p></div><div class="scenario-branches"><div class="scenario-origin"><span class="eyebrow">The starting point</span><h3>One country<br>Several plausible paths</h3><p>Compare alternatives over a defined horizon. Explain which observations would favour one over another.</p></div><div class="scenario-options">'''+items+'''</div></div><p class="visual-caption">A qualitative explanation of the scenario framework, not today’s forecast. Numerical probabilities require a dated assessment with defined outcomes and rules for events that overlap.</p><a class="text-link" href="methodology.html#six-layers">How the analytical layers inform these judgments </a></section>'''
