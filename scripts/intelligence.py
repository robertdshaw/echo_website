...
def question_families():
    ...
    Each is written so a date settles it rather than an argument. Keep them
    specific. A family that cannot be settled is not a question, it is a topic.
    """
    return ('<section class="section container" id="question-families"><div class="section-heading"><div><div class="eyebrow">The question set</div><h2>Six families of question,<br>written so a date settles them</h2></div><p>These are the operational families for an energy asset. Each carries a window, and a rule agreed in advance for what counts as settled.</p></div><div class="standards-grid"><div><span class="eyebrow">F1</span><h3>Control</h3><p>Will OFAC amend, revoke or let lapse the licence under which Chevron operates Petropiar, before a date in the window?</p></div><div><span class="eyebrow">F2</span><h3>Operations</h3><p>Will the José complex lose grid power for more than 24 hours in any single week, before a date in the window?</p></div><div><span class="eyebrow">F3</span><h3>Security</h3><p>Will armed forces deploy to, or take access control at, the José complex?</p></div><div><span class="eyebrow">F4</span><h3>Community</h3><p>Will a blockade of access to José last 12 hours or more?</p></div><div><span class="eyebrow">F5</span><h3>Local politics</h3><p>Will the regional PDVSA Oriente management, or the governor of Anzoátegui, be replaced?</p></div><div><span class="eyebrow">F6</span><h3>Money</h3><p>Will workers or contractors at José or Punta de Mata report arrears of four weeks or more?</p></div></div></section>')
