...
def question_families():
    ...
    """
    return ('<section class="section container" id="question-families"><div class="section-heading"><div><div class="eyebrow">The question set</div><h2>Six families of question,<br>written so a date settles them</h2></div><p>These are the operational families for an energy asset. The threshold and the window are set with you, because a day without power matters to one operator and not to another. What does not change is that each one resolves against a rule agreed before the work starts.</p></div><div class="standards-grid"><div><span class="eyebrow">F1</span><h3>Control</h3><p>Will the licence or authorisation the operator works under be amended, revoked or allowed to lapse?</p></div><div><span class="eyebrow">F2</span><h3>Operations</h3><p>Will the plant lose power, feedstock or a process unit for long enough to interrupt loading?</p></div><div><span class="eyebrow">F3</span><h3>Security</h3><p>Will armed forces deploy to the site, or take control of access to it?</p></div><div><span class="eyebrow">F4</span><h3>Community</h3><p>Will access to the site or the route to port be blocked for long enough to hold a shipment?</p></div><div><span class="eyebrow">F5</span><h3>Local politics</h3><p>Will the regional management of the state operator, or the governor of the state, be replaced?</p></div><div><span class="eyebrow">F6</span><h3>Money</h3><p>Will workers or contractors on the site report sustained arrears?</p></div></div></section>')
