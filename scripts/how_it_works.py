...
<section class="section container" id="five-operations"><div class="section-heading"><div><div class="eyebrow">Inside the fifth step</div><h2>Five operations</h2></div><p>These take the decisions a good editor or intelligence analyst makes by instinct and make each one explicit, so they can run continuously, at scale, on live material, with people owning the criteria.</p></div><div class="standards-grid ops-grid">{operations}</div>{figure("machine-diagram")}</section>
'''
