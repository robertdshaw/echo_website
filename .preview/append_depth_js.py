...
document.querySelectorAll('[data-actor]').forEach(button => button.addEventListener('click', () => {
  ['title','role','evidence','gap'].forEach((key,index) => { document.getElementById('actor-'+key).textContent = actorRecords[button.dataset.actor][index]; });
...
document.querySelectorAll('[data-pathway]').forEach(button => button.addEventListener('click', () => {
  ['title','copy','record'].forEach((key,index) => { document.getElementById('pathway-'+key).textContent = pathwayRecords[button.dataset.pathway][index]; });
