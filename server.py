...
FIELDS = {
    ...
    "question": ("Decision or question", 3000, True),
    "details": ("Additional details", 3000, False),
    "referral": ("How they found EchoFrame", 80, False),
}
...
def mail_configured(config):
    if not valid_email(config.get("CONTACT_FROM", "")) or not valid_email(
        config.get("CONTACT_TO", "")
    ):
        return False
    return bool(
        ...
        or (
            ...
            and config.get("SMTP_SECURITY") in ("ssl", "starttls")
        )
    )
...
def send_contact(config, values, request_id):
    ...
    body = "New EchoFrame website request\n\n" + "\n".join(
        f'{label}: {values[key] or "Not specified"}'
    ...
    if config.get("RESEND_API_KEY"):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "Idempotency-Key": "contact-" + request_id,
            },
            method="POST",
        )
    ...
    if config.get("SENDGRID_API_KEY"):
        ...
        payload = {
            "personalizations": [{"to": [{"email": config["CONTACT_TO"]}]}],
    ...
    message["From"] = config["CONTACT_FROM"]
    ...
    connection = (
        smtplib.SMTP_SSL(config["SMTP_HOST"], port, timeout=20, context=context)
        if config.get("SMTP_SECURITY") == "ssl"
        else smtplib.SMTP(config["SMTP_HOST"], port, timeout=20)
    )
...
def create_app(overrides=None, sender=None):
    ...
    app.config.update(
        ...
        CONTACT_TO="robert@echoframe.co",
        ...
        STATE_PATH=ROOT / ".contact-state" / "requests.sqlite3",
    ...
    def page_users():
        ...
        pairs = [
            pair.split(":", 1)
            for pair in os.getenv("BUREAU_USERS", "").split(",")
            if ":" in pair
        ]
        return {
            name.strip(): password
    ...
    def page_is_private():
        return (
            request.path in PRIVATE_PAGES
            and (PRIVATE_DIR / "frame-bureau-full.html").exists()
        )
    ...
    def private_page():
        ...
        if expected and hmac.compare_digest(auth.password or "", expected):
            return None
        return (
            "This page is private while it is being reviewed.",
            401,
            {
    ...
    def digest(text):
        return hmac.new(
            app.config["APP_SECRET"].encode(), text.encode(), hashlib.sha256
        ).hexdigest()

    @contextmanager
    def db():
        path = Path(app.config["STATE_PATH"])
        path.parent.mkdir(parents=True, exist_ok=True)
        conn = sqlite3.connect(path, timeout=5)
        conn.execute(
            "CREATE TABLE IF NOT EXISTS requests (id TEXT PRIMARY KEY, digest TEXT, state TEXT, created REAL)"
        )
    ...
    def contact():
        ...
        try:
            ...
            valid = 0 <= age < 7200 and hmac.compare_digest(
                signature, digest(stamp + "." + nonce)
            )
        ...
        if not valid:
            return (
                jsonify(
                    error="This form has expired. Reload the page; your request has not been sent.",
                    refresh=True,
                ),
                403,
            )
        ...
        for key, (label, limit, required) in FIELDS.items():
            ...
            if not isinstance(value, str):
                errors[key] = f"Please check {label.lower()}."
                continue
            value = value.strip()
            if (
                ...
                and key not in ("question", "details")
                or "\x00" in value
            ):
                errors[key] = (
                    f"Please enter {label.lower()} (up to {limit} characters)."
                )
            values[key] = value
        ...
        if errors:
            return (
                jsonify(error="Please check the highlighted details.", fields=errors),
                400,
            )
        request_id = data.get("request_id", "")
        if not isinstance(request_id, str) or not re.fullmatch(
            r"[a-f0-9-]{36}", request_id
        ):
            return jsonify(error="Please reload the form and try again."), 400
        if not mail_configured(app.config):
            return (
                jsonify(
                    error="Sending is temporarily unavailable. Your request has not been sent. Please try again later."
                ),
                503,
            )
        ...
        with db() as conn:
            ...
            conn.execute(
                "DELETE FROM requests WHERE created < ?", (time.time() - 86400,)
            )
            previous = conn.execute(
                "SELECT digest,state FROM requests WHERE id=?", (request_id,)
            ).fetchone()
            if previous:
                ...
                return (
                    jsonify(
                        error="This request is already being processed or delivery could not be confirmed. Please contact us before sending it again."
                    ),
                    409,
                )
            attempts = conn.execute(
                "SELECT count(*) FROM rate WHERE fingerprint=?", (fingerprint,)
            ).fetchone()[0]
            if attempts >= 5:
                return (
                    jsonify(
                        error="Too many requests. Please try again in an hour or use the contact email."
                    ),
                    429,
                )
            ...
            conn.execute(
                ...
                (request_id, payload_digest, "processing", time.time()),
            )
        try:
            ...
            with db() as conn:
                conn.execute(
                    ...
                    ("unconfirmed", request_id),
                )
            return (
                jsonify(
                    error="We could not confirm delivery. Your details are still here. Please contact us directly before trying again."
                ),
                502,
            )
        with db() as conn:
            conn.execute(
                "UPDATE requests SET state=? WHERE id=?", ("accepted", request_id)
            )
    ...
    def oversized(error):
        return (
            jsonify(error="Your message is too long. Please shorten it and try again."),
            413,
        )
    ...
    def private_bureau():
        ...
        return send_from_directory(
            PRIVATE_DIR, "frame-bureau-full.html", conditional=False
        )
